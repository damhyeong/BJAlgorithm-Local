...
1 초	512 MB	30082	7223	5374	23.006%
문제
...
public class Exam_24479 {
	public static boolean[] isVisited;
	public static List<ArrayList<Integer>> adjustList;
	...
	public static void main(String[] args) throws IOException {
		...
		N = Integer.parseInt(st.nextToken()); int M = Integer.parseInt(st.nextToken()); int R = Integer.parseInt(st.nextToken());
		...
		for(int i = 0; i < M; i++) {
			...
			// 연결되어 있음을 의미. 
			adjustList.get(num1).add(num2);
			
			adjustList.get(num2).add(num1);
		}
		
		for(int i = 0; i < adjustList.size(); i++) {
			
			Collections.sort(adjustList.get(i));
		}
	...
	public static void dfs(int start) {
		...
		for(int i = 0; i < adjustList.get(start).size(); i++) {
			int newVer = adjustList.get(start).get(i);
			if(!isVisited[newVer])
				dfs(newVer);
