...
public class Exam_1620 {
	...
	public static void main(String[] args) throws IOException {
		...
		HashMap<String, String> strMonsters = new HashMap<String, String>();
		HashMap<String, String> intMonsters = new HashMap<String, String>();
		
		for(int i = 0; i < N; i++) {
			String monsterName = br.readLine();
			strMonsters.put(monsterName, Integer.toString(i + 1));
			intMonsters.put(Integer.toString(i + 1), monsterName);
		}
		
		for(int i = 0; i < M; i++) {
			String question = br.readLine();
			
			if(strMonsters.containsKey(question))
				bw.write(strMonsters.get(question) + "\n");
			else 
				bw.write(intMonsters.get(question) + "\n");
		}
		bw.flush();
		bw.close();
		br.close();
