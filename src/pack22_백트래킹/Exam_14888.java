...
2 초	512 MB	89715	40859	27299	47.747%
문제
...
public class Exam_14888 {
	...
	public static boolean[] operIndexBool;
	public static int[] operIndex;
	public static int N;
	...
	public static void main(String[] args) throws IOException {
		...
		operIndexBool = new boolean[N - 1];
		operIndex = new int[N - 1];
	...
	public static void getOperIndex(int deep) {
		if(deep == N - 1) {
			getMinOrMax();
			return;
		}
		...
		for(int i = 0; i < N - 1; i++) {
			if(operIndexBool[i] == true)
				continue;
			else {
				operIndexBool[i] = true;
				operIndex[deep] = i;
				getOperIndex(deep + 1);
				operIndexBool[i] = false;
			}
		}
	}
	public static void getMinOrMax() {
		...
		if(!isFirst) {
			min = (min >= fNum) ? fNum : min;
			max = (max <= fNum) ? fNum : max;
		} else {
			isFirst = false;
			min = fNum;
			max = fNum;
