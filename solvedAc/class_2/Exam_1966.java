...
2 초	128 MB	68384	39199	30963	58.317%
문제
...
public class Exam_1966 {
	...
	public static void main(String[] args) throws IOException {
		...
		while(T --> 0) {
			...
			int K = Integer.parseInt(st.nextToken()); // 확인할 문서 순서 
			
			st = new StringTokenizer(br.readLine());
			for(int i = 0; i < N; i++) {
				if(i == K) {
					queue.add(new Document(Integer.parseInt(st.nextToken()), true));
				} else {
					queue.add(new Document(Integer.parseInt(st.nextToken()), false));
				}
			}
			int cnt = 0;
			while(true) {
				Iterator iter = queue.iterator();
				Document fDocument = (Document)(iter.next());
				
				boolean notImportant = false;
				
				while(iter.hasNext()) {
					Document newDocument = (Document)iter.next();
					if(fDocument.getImportant() < newDocument.getImportant()) {
						notImportant = true;
						break;
					}
				}
...
class Document{
	...
	public Document(int important, boolean isSelect) {
