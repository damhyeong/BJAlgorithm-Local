...
1 초	256 MB	174393	59806	50168	33.195%
문제
...
public class Exam_10250 {
	...
	public static void main(String[] args) throws IOException {
		...
		while(T --> 0) {
			...
			int height = Integer.parseInt(st.nextToken()); // 층 수 
			int width = Integer.parseInt(st.nextToken()); // 호 수 
			int N = Integer.parseInt(st.nextToken());
			room = new boolean[height][width];
			...
			checkH += 1; checkW += 1;
			if(checkW >= 10) {
				bw.write(Integer.toString(checkH) + Integer.toString(checkW) + "\n");
			} else {
				bw.write(Integer.toString(checkH) + "0" + Integer.toString(checkW) + "\n");
			}
		}
		bw.flush();
		bw.close();
